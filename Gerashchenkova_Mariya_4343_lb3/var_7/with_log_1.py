...
def main():
        ...
        for j in range(m + 1):
            ...
            if i < n and j < m and a[i] == b[j]:
                ...
                if new_cost < dp[i + 1][j + 1]:
                    ...
                    print("Клетка обновлена, в parent записана операция M.")
                else:
            ...
            if i < n and j < m and a[i] != b[j]:
                ...
                if can_replace(a, i, cursed, variant):
                    ...
                    if new_cost < dp[i + 1][j + 1]:
                        ...
                        print("Клетка обновлена, в parent записана операция R.")
                    else:
                        print("Клетка не обновлена.")
                else:
            ...
            if j < m:
                ...
                if new_cost < dp[i][j + 1]:
                    dp[i][j + 1] = new_cost
                    parent[i][j + 1] = (i, j, "I")
                    print("Клетка обновлена, в parent записана операция I.")
                else:
                    print("Клетка не обновлена.")
            ...
            if i < n:
                ...
                if can_delete(a, i, cursed, variant):
                    ...
                    if new_cost < dp[i + 1][j]:
                        dp[i + 1][j] = new_cost
                        parent[i + 1][j] = (i, j, "D")
                        print("Клетка обновлена, в parent записана операция D.")
                    else:
                        print("Клетка не обновлена.")
                else:
    ...
    print("\nНачинаем восстановление редакционного предписания.")
    print(f"Стартуем из клетки dp[{i}][{j}].")

    while i > 0 or j > 0:
        prev_i, prev_j, op = parent[i][j]

        print("-" * 60)
        print(f"Сейчас находимся в клетке dp[{i}][{j}].")
        print(f"Пришли сюда из dp[{prev_i}][{prev_j}] операцией {op}.")
    ...
    print("\nОперации были восстановлены в обратном порядке.")
    print("Разворачиваем список операций.")

    print("\nИтоговое редакционное предписание:")
    print("".join(operations))

    print("Исходная строка A:")
    print(a)

    print("Итоговая строка B:")
    print(b)
